// MongoDB API Service for Retinal-AI Platform (Browser-compatible)
// This service communicates with the backend API instead of connecting directly to MongoDB
...
class MongoDBService {
  ...
  async initializeDatabase() {
    try {
      ...
      if (response.ok) {
        console.log('MongoDB Backend API connection verified');
        this.isInitialized = true;
      } else {
        throw new Error('MongoDB Backend API not accessible');
      }
    } catch (error) {
      console.error('MongoDB Backend API verification error:', error);
      this.isInitialized = false;
    }
  }
  ...
  async getDoctorCategories() {
    ...
    try {
      const response = await fetch(`${this.baseUrl}/doctors/categories`, {
        headers: {
          'Authorization': `Bearer ${this.authToken}`
        }
      });
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to fetch doctor categories');
      }
  ...
  async startModelTraining(trainingConfig) {
    ...
    try {
      const response = await fetch(`${this.baseUrl}/model-training/start`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.authToken}`
  ...
  async getTrainingStatus(jobId) {
    ...
    try {
      const response = await fetch(`${this.baseUrl}/model-training/status/${jobId}`, {
        headers: {
          'Authorization': `Bearer ${this.authToken}`
        }
      });
  ...
  async stopModelTraining(jobId) {
    if (!this.authToken) {
      throw new Error('Authentication required');
    }

    try {
      const response = await fetch(`${this.baseUrl}/model-training/stop/${jobId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${this.authToken}`
        }
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to stop model training');
      }

      const result = await response.json();
      return result;
    } catch (error) {
      console.error('Stop model training error:', error);
      throw error;
    }
  }
